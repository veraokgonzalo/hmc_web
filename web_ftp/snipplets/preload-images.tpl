	...
	{% if settings.home_order_position_1 == 'featured_banners' %}
        ...
        {% for banner in ['banner_01', 'banner_02', 'banner_03', 'banner_04'] %}
            ...
            {% if banner_show and banner_image and not priority_assigned %}
				...
				<link rel="preload" fetchpriority="high" as="image" href="{{ banner_image_src | settings_image_url('large') }}" imagesrcset="{{ banner_image_src | settings_image_url('large') }} 480w, {{ banner_image_src | settings_image_url('huge') }} 640w, {{ banner_image_src | settings_image_url('original') }} 1024w, {{ banner_image_src | settings_image_url('1080p') }} 1920w">
			{% endif %}
		{% endfor %}
	{% endif %}
...
{% elseif template == 'product' %}
    ...
    {% for image in product.images %}
        {% if loop.first %}
            <link rel="preload" fetchpriority="high" as="image" href="{{ image | product_image_url('large') }}" imagesrcset="{{ image | product_image_url('large') }} 480w, {{ image | product_image_url('huge') }} 640w, {{ image | product_image_url('original') }} 1024w, {{ image | product_image_url('1080p') }} 1920w, {{ image | product_image_url('1440p') }} 2560w, {{ image | product_image_url('4k') }} 3840w">
        {% endif %}
    {% endfor %}
