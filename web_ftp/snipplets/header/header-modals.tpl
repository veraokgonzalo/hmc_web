...
{% if not store.is_catalog and settings.ajax_cart and template != 'cart' %}           
	...
	{% if settings.add_to_cart_recommendations %}
		...
		{% embed "snipplets/modal.tpl" with{modal_id: 'related-products-notification', modal_header_title: true, modal_class: 'bottom modal-overflow-none modal-bottom-sheet h-auto', modal_position: 'bottom', modal_transition: 'slide', modal_footer: false, modal_width: 'centered-md modal-centered-md-600px', modal_body_class: 'modal-scrollable'} %}
			{% block modal_head %}
