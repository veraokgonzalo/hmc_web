<div class="js-product-variants {% if quickshop %}js-product-quickshop-variants{% endif %} row {% if not settings.bullet_variants %}mb-3{% endif %}">
	...
	{% for variation in product.variations %}
		...
		{% if settings.image_color_variants and not (settings.bullet_variants)  %}
			...
			{% endif %}
		{% endif %}
		<div class="js-product-variants-group {% if variation.name in ['Color', 'Cor'] %}js-color-variants-container{% endif %} col-12 {% if settings.bullet_variants %}form-group{% endif %} {% if show_size_guide and settings.size_guide_url and has_size_variations and loop.last %}mb-2{% endif %}" data-variation-id="{{ variation.id }}">
			{% if quickshop %}
			...
			{% else %}
				...
				{% endembed %}
			{% endif %}
			{% if settings.bullet_variants or (settings.image_color_variants and variation.name in ['Color', 'Cor']) %}
	...
	{% if show_size_guide and settings.size_guide_url and has_size_variations %}
		...
		{% for page in pages if page.handle == size_guide_url_handle and not has_size_guide_page_finded %}
			...
			{% if has_size_guide_page_finded %}
				<a data-toggle="#size-guide-modal" data-modal-url="modal-fullscreen-size-guide" class="js-modal-open js-fullscreen-modal-open font-small col-12 pb-2 {% if settings.bullet_variants %}mt-1 mb-3{% endif %}">
					{% include "snipplets/svg/ruller.tpl" with {svg_custom_class: "icon-inline icon-lg svg-icon-text mr-1"} %}
					<span class="btn-link no-underline font-smallest">{{ 'Guía de talles' | translate }}</span>
				</a>
