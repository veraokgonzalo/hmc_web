...
> **📦 Archivo histórico:** este log documenta trabajo ya completado sobre la estructura de tema con sections/blocks (`web_fork/`), eliminada del repo. Se conserva sin modificar como registro de lo hecho — no es una guía para trabajo futuro, que ahora se hace sobre `web_ftp/` (ver `CLAUDE.md` y `docs/progress.md`).
